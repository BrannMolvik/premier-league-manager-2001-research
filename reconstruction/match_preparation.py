...
def prepare_ai_match_selection(
    ...
    *,
    ...
    Competition-specific registration/cup-tie checks and the bit-11 quota are
    still supplied externally through callbacks. The unusual 0x41FA50
    secondary-team exception in the original clear loop is likewise represented
    by preserve_existing_selection until that secondary team field is named.
    """
    ...
    if require_complete_xi and result.unfilled_slot_indices:
        raise ValueError(
            "AI lineup core could not fill all 11 formation slots; "
            "original restriction relaxation/context handling is still required"
        )
